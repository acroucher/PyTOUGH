...
class linear_trans2(object):
    ...
    def between_rects(self,rect1,rect2):
        """Returns a linear transformation mapping one rectangle to another.  Each rectangle should be a
        list or tuple of two 2-D numpy arrays (bottom left and top right corners)."""
        tolerance=1.e-6
        d1=rect1[1]-rect1[0]
    ...
    def between_points(self,points1,points2):
        """Returns a least-squares best fit linear transformation between two sets of points (e.g. the 
        same set of points in two different coordinate systems).
        The parameters points1 and points2 should contain lists of the corresponding points in the first
        and second coordinate systems respectively.  At least three points are needed to find a best-fit
        ...
        if npts1==npts2:
            ...
            if npts>=3:
                ...
                N1=linear_trans2(np.array([[1./dx1[0],0.],[0.,1./dx1[1]]]),-np.array([r1[0][0]/dx1[0],r1[0][1]/dx1[1]]))
                N2=linear_trans2(np.array([[1./dx2[0],0.],[0.,1./dx2[1]]]),-np.array([r2[0][0]/dx2[0],r2[0][1]/dx2[1]]))
                ...
                for p1,p2 in zip(n1,n2):
                    ...
                    r+=np.array([p1[0]*p2[0],p1[1]*p2[0],p1[0]*p2[1],p1[1]*p2[1],p2[0],p2[1]])
                try:
    ...
    def rotation(self,angle,centre=np.zeros(2)):
        ...
        about an optionally specified centre."""
