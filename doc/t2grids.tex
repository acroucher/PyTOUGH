...
Creates a \hyperref[mulgrids]{\texttt{mulgrid}} geometry object from a rectangular TOUGH2 grid, together with a dictionary defining the mapping between the geometry block names and the grid block names. This block mapping can be used when the naming convention used by the original TOUGH2 grid is not compatible with the layer/column naming conventions assumed by a \texttt{mulgrid} geometry.
...
\item \textbf{origin\_block}: string, \hyperref[t2blockobjects]{\texttt{t2block}} or \texttt{None}\\
  ...
  \item \textbf{atmos\_volume}: float\\
    Maximum block volume for blocks to be considered part of the geometrical grid. Blocks with volume greater than this will be assumed o be boundary condition blocks and will not be represented geometrically.
