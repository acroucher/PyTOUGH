...
Note that a table row contains only the numerical values stored in that row, and not the corresponding key value (e.g. block name).  (This is because the data are stored internally as an \texttt{np.array} of floating point values, for efficiency with large tables.)  The key values of a \texttt{listingtable} may be accessed via the \texttt{row\_name} property, which is a list containing the key value for each row.  The column headings of the table can similarly be accessed via the \texttt{column\_name} list property.  The \texttt{num\_rows} and \texttt{num\_columns} properties of a \texttt{listingtable} object return the numbers of rows and columns respectively.
