...
\subsubsection{\texttt{node\_nearest\_to(\emph{point})}}

Returns the node nearest to a specified point.
...
Refines the specified columns in the grid.  Appropriate transition columns are created around the refined region.  If no columns are specified, all columns are refined.  All columns in the region to be refined (and in the transition region) must be either triangular or quadrilateral.  Each column in split into four, unless the \emph{bisect} parameter is \texttt{True}, in which case each column in split into two.  If \emph{bisect} is `x' or `y', columns are split in the closest direction to the axis specified; or if \emph{bisect} is \texttt{True}, between its longest sides.
