...
\begin{longtable}{|l|l|p{70mm}|}
  ...
  \hyperref[sec:mulgrid:amesh]{\texttt{amesh}} & (\hyperref[mulgrids]{\texttt{mulgrid}}, dict) & creates Voronoi geometry from AMESH data\\ 
...
\begin{snugshade}\subsubsection{\texttt{amesh(\emph{input\_filename}='in', \emph{segment\_filename}='segmt', \emph{convention}=0, \\
      \emph{node\_tolerance}=None, \emph{justify}='r', \emph{chars}=ascii\_lowercase, \emph{spaces}=\texttt{True})}}\end{snugshade}
\label{sec:mulgrid:amesh}
...
\item \textbf{node\_tolerance}: float or \texttt{None}\\
  Horizontal tolerance for identifying distinct nodes in the segment file. If a node is read in with horizontal distance from an existing node less than the tolerance, then the two nodes are assumed to be identical. If \texttt{None} (the default), then the tolerance is set to 90\% of the smallest segment length.
