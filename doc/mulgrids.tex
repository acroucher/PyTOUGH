...
\subsubsection{\texttt{layer\_plot(\emph{layer}, \emph{variable}=None, \emph{variable\_name}=None, \emph{unit}=None,\\
    ...
    \emph{well\_names}=True, \emph{show\_outside\_wells}=True, \emph{wellcolour}=`blue', \emph{welllinewidth}=1.0)}}
...
Well tracks can also optionally be plotted.  Each well is drawn as a line following the well track, with the well name at the bottom of the well, and cross at the point where the well track intersects the layer centre (if it does).
...
\item \textbf{show\_outside\_wells}: Boolean\\
  Set to \texttt{False} if wells that do not intersect the specified layer are not to be shown.
