...
\begin{longtable}{|l|l|p{70mm}|}
  ...
  \hyperref[sec:mulgrid:fit_columns]{\texttt{fit\_columns}} & \texttt{np.array} & fits scattered data to column centres\\ 
...
\begin{snugshade}\subsubsection{\texttt{fit\_columns(\emph{data}, \emph{alpha}=0.0, \emph{beta}=0.0, \emph{columns}=[], \emph{min\_columns}=[], \\
    \emph{grid\_boundary}=False, \emph{silent}=False)}}\end{snugshade}
...
This method returns an \texttt{np.array} with length given by the number of columns in the geometry. Each value in the array represents the fitted data value at the centre of the corresponding column.
