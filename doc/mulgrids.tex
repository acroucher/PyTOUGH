...
when the \texttt{mulgrid} object is created or read from file, as an optional parameter.
It can also be specified after creation. The \texttt{block\_order} property is a string which can
take the value `layer\_column' for the default layer/column block ordering, or `dmplex' if
the blocks are to be sorted by geometrical type.
...
\begin{snugshade}\subsubsection{\texttt{from\_amesh(\emph{input\_filename}='in', \emph{segment\_filename}='segmt', \emph{convention}=0, \\
      ...
      \emph{block\_order}=`layer\_column')}}\end{snugshade}
...
\item \textbf{block\_order}: string\\
  Specify `layer\_column' for default block ordering by layer and column, starting from the atmosphere. Specify `dmplex' to order blocks by geometrical type (8-node hexahedrons first followed by 6-node wedges) as in PETSc DMPlex meshes.
\end{itemize}
...
\subsubsection{\texttt{rectangular(\emph{xblocks}, \emph{yblocks}, \emph{zblocks}, \emph{convention}=0, \emph{atmos\_type}=2,\\
    ...
    \emph{spaces}=\texttt{True}, \emph{block\_order}=`layer\_column'})}\end{snugshade}
